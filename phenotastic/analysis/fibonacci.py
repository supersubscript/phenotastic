...
outdir = '/home/henrik/out_fib_comparison'
...
################################################################################
    ...
    A.mesh = A.mesh.Decimate(0.95, volume_preservation=True, normals=True, inplace=False)
...
################################################################################
    ...
    pdata = boa.merge_boas_disconnected(A, pdata, meristem_index, threshold=.3, threshold2=.1)
    boas, boasData = boa.get_boas(pdata)
    print boa.nboas(pdata)
...
################################################################################
    ...
    pdata = boa.merge_boas_disconnected(A, pdata, meristem_index, threshold=.2, threshold2=.1)
    boas, boasData = boa.get_boas(pdata)
    print boa.nboas(pdata)
...
###############################################################################
    ...
    pobj.AddPointLabels(
            ...
            fontsize=30, pointcolor='w', textcolor='w')
    pobj.Plot(in_background=False, autoclose=False, interactive=False)
    pobj.TakeScreenShot(outdir + '/figs/' + os.path.splitext(os.path.basename(file_))[0] + '_bottom.png')
    pobj.Close()x
