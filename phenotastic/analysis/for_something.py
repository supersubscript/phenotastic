...
#    A.mesh.Plot()
    ...
    A.mesh.ClipPlane([np.ceil(bounds[0]), 0, 0], [1, 0, 0])
    A.mesh.ClipPlane([np.floor(bounds[1]), 0, 0], [-1, 0, 0])
    A.mesh.ClipPlane([0, np.ceil(bounds[2]), 0], [0, 1, 0])
    A.mesh.ClipPlane([0, np.floor(bounds[3]), 0], [0, -1, 0])
    A.mesh.ClipPlane([0, 0, np.ceil(bounds[4])], [0, 0, 1])
    A.mesh.ClipPlane([0, 0, np.floor(bounds[5])], [0, 0, -1])
    A.mesh.ExtractLargest()
    ...
    bounds = A.mesh.GetBounds()
    A.mesh.ClipPlane([bounds[0] + 20, 0, 0], [1,0,0])
    A.mesh.FillHoles(100.0)
    A.compute_normals()
...
#    A.mesh.RotateZ(-45)
#    A.mesh.RotateY(-90)
#    A.mesh.Plot(color='orange',background='w')
#
...
#
#    pobj = vi.PlotClass()
#    pobj.AddMesh(A.mesh, scalars=pdata['domain'].values, opacity=.5)
#
#    if len(boaCoords) > 0:
#        pobj.AddPointLabels(np.array(boaCoords), np.array([str(ii) for ii in xrange(
#            len(boaCoords))]), fontsize=50, pointcolor='w', textcolor='white', shadow=True)
#
#    for ii in xrange(len(boaCoords)):
#        pobj.AddMesh(boa.get_domain_boundary(A.mesh, pdata, ii),
#                     color='black', psize=10, linethick=10)
#    pobj.SetBackground('white')
##    pobj.AddPoints(apexcoords, color='red', psize=50)
#    pobj.AddPoints(np.array([np.array(ii) for ii in ddata.com.values]), color='red', psize=25)
#
#    pobj.Plot()
