...
A = ap.AutoPhenotype()
A.data = fluo.copy()
#A.data = dnl(A.data, patch_distance=5, patch_size=3)

''' Process data before creating contour. '''
A.data = equalize_hist(A.data, mask=A.data > np.mean(A.data) * 1.0)
A.data = dnl(A.data, patch_distance=3, patch_size=2)
#tiff.imshow(A.data)
#pl.PlotImage(A.data, xyzres, mask=contour, opacity=.1, psize=.5)
#A.data = dnl(A.data, patch_distance=2, patch_size=3)
#A.data = dnl(A.data, patch_distance=2, patch_size=3)
#A.data = dnl(A.data, patch_distance=2, patch_size=3)

for ii in xrange(1):
    A.data = median_filter(A.data, size=3)
for ii in xrange(1):
    A.data = gaussian_filter(A.data, sigma=[1, 2, 2])

import skimage
#A.data = skimage.restoration.denoise_tv_bregman(A.data, 1, max_iter=10, eps=0.001, isotropic=True)
tiff.imshow(A.data)
#A.data[A.data < 0.4] = 0
...
#A.data[A.data < 3*np.mean(A.data)] = 0
...
#        A.data, 4, init_level_set=A.data > factor * np.mean(A.data),
#        smoothing=3, threshold='auto', balloon=0)
tiff.imshow(contour)
# tiff.imshow(A.data)
...
#contour = binary_opening(contour, structure=np.ones((3,3,3)), iterations=1)
#tiff.imshow(contour)
...
A.mesh.ClipPlane([first_center * z - 38 * z, 0, 0], [xyzres[0], 0, 0])
...
var = 'domain'
pointData = pdata
pobj.AddMesh(A.mesh, scalars=pointData[var].values)

if len(boaCoords) > 0:
    pobj.AddPointLabels(np.array(boaCoords), np.array([str(ii) for ii in xrange(
        len(boaCoords))]), fontsize=50, pointcolor='w', textcolor='black')

bcolor = 'black'
blinewidth=10
bpsize=10
for ii in xrange(len(boaCoords)):
    pobj.AddMesh(boa.get_domain_boundary(A.mesh, pointData, ii),
                 color=bcolor, psize=bpsize, linethick=blinewidth)

pobj.SetBackground([1,1,1])
pobj.Plot()
...
angle_threshold = 7.5
pdata, ddata = boa.merge_boas_angle(pdata, ddata, A.mesh, angle_threshold, apex, meristem_index)
