...
from clahe import _clahe_impl

def clahe(img, win_shape, clip_limit, _fast=True, **kwargs):
    ...
    if _fast:
        if img.ndim == 2:
            ...
            res = _clahe_impl.clahe(
                img[..., None].astype(np.uint16), *win_shape, clip_limit=clip_limit)[..., 0]
        elif img.ndim == 3:
            res = _clahe_impl.clahe(img.astype(np.uint16),
                                    *win_shape,
