...
def tiffload(fname):
    ...
    if 'scaninformation' in metadata and isinstance(metadata['scaninformation'], str):
        fobj.metadata['scaninformation'] = eval(fobj.metadata['scaninformation'])
...
#            y = 1.0
    try:
        xindex = np.where(np.array(list(fobj.pages.pages[0].tags.keys())) == 'XResolution')[0][0]
        x = list(fobj.pages.pages[0].tags.values())[xindex].value
        x = float(x[1]) / x[0]
    except:
        try:
            x = fobj.metadata['voxelsizex']
            ...
            x = 1.0
    try:
        yindex = np.where(np.array(list(fobj.pages.pages[0].tags.keys())) == 'YResolution')[0][0]
        y = list(fobj.pages.pages[0].tags.values())[yindex].value
        y = float(y[1]) / y[0]
    except:
        try:
            y = fobj.metadata['voxelsizey']
...
def tvf2si(fobj, reduce_dims=[]):
    ...
    if len(reduce_dims) > 0:
        ...
        for ii in range(len(reduce_dims)):
